...
const username = "exampleyahoomail@yahoo.com";
...
test('Yahoo login test', async ({page}) => {  
    ...
    await page.locator('#login-passwd').press('Enter');
    ...
    if (currentURL.includes('yahoo.com/enable-fingerprint')) {
        ...
        await expect(page.locator('#ybarMailLink')).toBeVisible({ timeout: 15000 });
    ...
    await expect(page.locator('#ybarMailLink')).toBeVisible({ timeout: 15000 });
    await expect(page.locator('#ybarMailLink')).toHaveAttribute('href', /mail\.yahoo\.com/i);
    const mailHref = await page.locator('#ybarMailLink').getAttribute('href');
    await page.locator('#ybarMailLink').click();
...
//Run in terminal: npx playwright test tests/08_yahoo_login/03_yahoo_login_hardcoded_values.spec.js --headed